...
console.log('Table Name:', GURUDWARA_EVENTS_TABLE);
...
const adminUpdateEvent: APIGatewayProxyHandler = async (event: APIGatewayEvent) => {
  try {
    ...
    console.log('Get Command:', getCommand);
    ...
    if (updateData.organizer?.image) {
      ...
      if (!Array.isArray(organizerUploadResult)) {
        return formatJSONResponse({
          ...
          errors:
            typeof organizerUploadResult.body === 'string'
              ? organizerUploadResult.body
              : JSON.stringify(organizerUploadResult.body),
        });
      }
    ...
    console.log('DynamoDB Update Command:', JSON.stringify(command, null, 2));
